...
WHERE ROUTINE_NAME = 'USP_RestoreSGEN') 
...
BEGIN
	...
	insert into #BKPDET
	SELECT top 30
		BS.backup_finish_date, physical_device_name, RIGHT(physical_device_name, CHARINDEX('\', REVERSE(physical_device_name)) -1)  [file_name]
	FROM msdb..backupset BS
		JOIN MSDB..backupmediafamily BMF ON BS.media_set_id=BMF.media_set_id
	WHERE type='D'
		AND database_name=@DBName
	ORDER BY backup_finish_date DESC
	SELECT @@LBKP=BKP_FINISH_DATE, @@ORGINALFULLPATH=Original_full_path
	FROM #BKPDET
	WHERE RNO=@LastFull
	SELECT @@BKPDETCOUNT=COUNT(*)
	FROM #BKPDET
