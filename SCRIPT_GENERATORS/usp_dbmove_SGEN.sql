...
BEGIN
	...
	SELECT name
			...
			where db_name(database_id)=@DBNAME and type_desc='ROWS'
			OPEN CSR_SMF
			FETCH NEXT FROM CSR_SMF INTO @@NAME
			WHILE @@FETCH_STATUS = 0   
	BEGIN
				--Print old path
				SELECT @@OldDataPath = LEFT(physical_name,LEN(physical_name) - charindex('\',reverse(physical_name),1)		+ 1) , @@Datafilename =RIGHT(physical_name, CHARINDEX('\', REVERSE(physical_name)) -1)
				from sys.master_files
			...
			--Move files at OS
			select @@PhysicalDatafileCopy= 'XCOPY '''+ physical_name +''' '''+@NewDataPath+''' /Y /I /F'
...
BEGIN
			...
			SELECT @@OldlogPath = LEFT(physical_name,LEN(physical_name) - charindex('\',reverse(physical_name),1) + 1) ,
				@@logfilename =RIGHT(physical_name, CHARINDEX('\', REVERSE(physical_name)) -1)
			from sys.master_files
			...
			select @@PhysicallogfileCopy= 'XCOPY '''+ physical_name +''' '''+@NewLogPath+''' /Y /I /F'
			from master.sys.master_files
			where db_name(database_id) =@DBName and type_desc='LOG'
