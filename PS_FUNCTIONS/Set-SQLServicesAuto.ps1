    ...
    foreach ($cname in $computername) {
        try { 
            ...
            foreach ($eachsqlser in $sqlservices) {
                if ($eachsqlser.startmode -eq 'Manual') {
                    $servicename = $eachsqlser.name
                    Set-Service -ComputerName $cname -Name $servicename -StartupType Automatic
                    Write-Verbose "Changed StartMode to Automatic for service: $servicename"
                }
            }

            Write-Output "`nAfter Startup change"
            $sqlservices = gwmi win32_service -Filter "displayname like 'SQL%' and not StartMode='Disabled' and not name='SQLWriter'" -ComputerName $cname -ErrorAction stop
            $sqlservices | select __server, name, displayname, state, startmode, startname|  Export-Csv "$wdir\SetSQLStartupAuto_$printdate.csv"  -NoTypeInformation -force -append
            $sqlservices | select __server, name, displayname, state, startmode, startname |ft -auto | Format-Color @{'Running' = 'Green'; 'stopped' = 'RED'}
        }
        catch {
            "Error checking for SQL Services in host $cname, reason below" | Out-File "$wdir\Errors_SetSQLStartupAuto_$printdate.txt" -Append
            $errout = Test-Connection -ComputerName $cname -Count 1 -EA SilentlyContinue 
            if (!$errout)
            {"**Unable to ping the Server: $cname" | Out-File "$wdir\Errors_SetSQLStartupAuto_$printdate.txt" -Append}
            else
            {"**$env:USERNAME does not have access to $cname" | Out-File "$wdir\Errors_SetSQLStartupAuto_$printdate.txt" -Append}
        }
